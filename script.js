...
const postmethods = () => {
  // Load the JSON data from amazonSearchResults.json
  fetch('amazonSearchResults.json')
    .then((response) => response.json())
    .then((amazonSearchResults) => {
      amazonSearchResults.forEach((postData) => {
        const postElement = document.createElement('div');
        postElement.classList.add('new-card');
        postElement.innerHTML = `
          <img src="${postData.imageLink}" class="card-img-top" alt="...">
          <div class="card-body d-flex flex-column justify-content-center">
            <h5 class="card-title text-white">${postData.title}</h5>
            <p class="card-text text-white">${postData.price}</p>
            <a href="#" class="btn btn-primary mt-auto">Add to Cart</a>
...
function searchCards(query) {
    ...
    fetch('amazonSearchResults.json')
        .then((response) => response.json())
        .then((amazonSearchResults) => {
            const filteredCards = amazonSearchResults.filter((card) =>
